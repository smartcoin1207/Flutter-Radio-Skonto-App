...
class CarouselSliderState extends State<CustomCarouselSlider>
  ...
  Widget getEnlargeWrapper(Widget? child,
    ...
    if (widget.options.enlargeStrategy == CenterPageEnlargeStrategy.zoom) {
      scale = pow(scale!, 4.0 - itemOffset.abs()).toDouble();

      child = Opacity(child: child, opacity: scale);

      //print(itemOffset.abs());

      return Transform.scale(
          child: Transform.translate(
            offset: itemOffset == 0 ? Offset(0.0, 0.0) : Offset(itemOffset > 0 ? pow(2 - scale, 10.0).toDouble() :  - pow(2 - scale, 10.0).toDouble(), 0.0),
            child: child,
          ),
