...
class CustomScrollSnapList extends StatefulWidget {
  ///List background
  ...
  @override
  CustomScrollSnapListState createState() => CustomScrollSnapListState();
}

class CustomScrollSnapListState extends State<CustomScrollSnapList> {
  ...
  double calculateScale(int index) {
    ...
    //default equation
    return 1 - min(difference.abs() / 250, 0.6);
  }
  ...
  Widget _buildListItem(BuildContext context, int index) {
    ...
    if (widget.dynamicItemSize) {
      child = Transform.scale(
        ...
        child: widget.itemBuilder(context, index, calculateScale(index)),
      );
    } else {
      child = widget.itemBuilder(context, index, calculateScale(index));
    }
